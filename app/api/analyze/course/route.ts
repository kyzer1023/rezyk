...
import {
  ...
  COURSE_ANALYSIS_GENERATION_PROFILE,
  CourseAnalysisGenerationSchema,
  type CourseAnalysisOutput,
  type StoredCourseAnalysis,
} from "@/lib/analysis/course-analysis-schema";
import { buildCourseAnalysisPrompt, type CourseAnalysisPromptInput } from "@/lib/analysis/course-analysis-prompt";
...
export async function POST(req: Request) {
  try {
    ...
    if (completedQuizIds.length === 0) {
      const result: StoredCourseAnalysis = {
        courseId,
        analyzedQuizIds: [],
        generatedAt: Date.now(),
        modelId: COURSE_ANALYSIS_MODEL_ID,
        status: "insufficient_data",
        error: "No completed quiz analyses found for this course.",
        ownerId: session.sub,
      };
      await adminDb.collection("courseAnalyses").doc(courseId).set(result);
    ...
    const quizSummaries: CourseAnalysisPromptInput["quizSummaries"] = [];
    ...
    for (const quizWorkId of completedQuizIds) {
      ...
      quizSummaries.push({
        ...
        conceptHeatmap: (derived.conceptHeatmap ?? []).map((c: Record<string, unknown>) => ({
          concept: c.concept as string,
          affectedStudentCount: c.affectedStudentCount as number,
          dominantErrorType: c.dominantErrorType as string,
        })),
        errorTypeBreakdown: derived.errorTypeBreakdown ?? [],
      });
    }

    const promptInput: CourseAnalysisPromptInput = {
      courseName,
      courseId,
      quizSummaries,
    };

    const prompt = buildCourseAnalysisPrompt(promptInput);
    const client = new GoogleGenAI({ apiKey: resolveApiKey() });

    const response = await client.models.generateContent({
      model: COURSE_ANALYSIS_MODEL_ID,
      contents: prompt,
      config: {
        responseMimeType: COURSE_ANALYSIS_GENERATION_PROFILE.responseMimeType,
        responseJsonSchema: CourseAnalysisGenerationSchema,
        temperature: COURSE_ANALYSIS_GENERATION_PROFILE.temperature,
        topP: COURSE_ANALYSIS_GENERATION_PROFILE.topP,
        maxOutputTokens: COURSE_ANALYSIS_GENERATION_PROFILE.maxOutputTokens,
      },
    });

    const rawText = typeof response.text === "string" ? response.text : String(response.text ?? "");
    const analysis: CourseAnalysisOutput = JSON.parse(rawText);
    ...
    return NextResponse.json({ success: true, status: "success", analysis });
  } catch (e) {
...
export async function GET(req: Request) {
  try {
    ...
    const snap = await adminDb.collection("courseAnalyses").doc(courseId).get();
    if (!snap.exists) {
