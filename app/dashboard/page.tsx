...
export default function DashboardPage() {
  ...
  const recentQuiz = mockQuizzes.find(
    (quiz) =>
      quiz.courseId === lastCourse.id && quiz.analysisStatus === "completed",
  );
  const analyzedCount = mockQuizzes.filter(
    (quiz) => quiz.analysisStatus === "completed",
  ).length;
  ...
  return (
    <div>
      ...
      <p
        className="edu-fade-in edu-fd1 edu-muted"
      ...
      >
        ...
        <div className="edu-card" style={{ padding: 24 }}>
          ...
          {["Google Classroom", "Google Forms", "Gemini AI"].map(
            (statusItem) => (
              <div
                key={statusItem}
                style={{
      ...
      {recentQuiz && (
        <div
          className="edu-card edu-fade-in edu-fd3"
        ...
        >
          ...
          <p className="edu-muted" style={{ fontSize: 14, marginBottom: 12 }}>
            {recentQuiz.title} &mdash; {recentQuiz.responseCount}/
            {recentQuiz.totalStudents} responses
          </p>
