...
const navItems = [
  { href: "/3/dashboard", label: "Overview", icon: "M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" },
  { href: "/3/dashboard/courses", label: "Courses", icon: "M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" },
  { href: "/3/settings", label: "Settings", icon: "M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z" },
];

function buildBreadcrumbs(pathname: string): { label: string; href: string }[] {
  const crumbs: { label: string; href: string }[] = [{ label: "Dashboard", href: "/3/dashboard" }];
  if (pathname.includes("/courses")) {
    crumbs.push({ label: "Courses", href: "/3/dashboard/courses" });
    const courseMatch = pathname.match(/courses\/([^/]+)/);
    if (courseMatch) {
      const cId = courseMatch[1];
      crumbs.push({ label: "Course", href: `/3/dashboard/courses/${cId}` });
      if (pathname.includes("/quizzes")) {
        crumbs.push({ label: "Quizzes", href: `/3/dashboard/courses/${cId}/quizzes` });
        ...
        if (quizMatch) {
          ...
          if (pathname.includes("/sync")) crumbs.push({ label: "Sync", href: pathname });
            ...
            crumbs.push({ label: "Students", href: `/3/dashboard/courses/${cId}/quizzes/${qId}/students` });
...
export default function R3DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    <div style={{ display: "flex", minHeight: "100vh" }}>
      ...
      >
        <Link href="/3" style={{ textDecoration: "none" }}>
          <div style={{ padding: "0 24px 18px", borderBottom: "1px solid #F0ECE5" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span className="r3-heading" style={{ fontSize: 16, fontWeight: 600, color: "#C17A56" }}>EduInsight</span>
            </div>
          </div>
        </Link>
        ...
        <nav style={{ flex: 1, padding: "14px 12px" }}>
          {navItems.map((item) => {
            const active = pathname === item.href || (item.href !== "/3/dashboard" && pathname.startsWith(item.href));
            return (
          ...
          {breadcrumbs.map((crumb, i) => (
            <span key={crumb.href} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              {i === breadcrumbs.length - 1 ? (
                ...
                <Link href={crumb.href} style={{ color: "#B5AA9C", textDecoration: "none" }}>{crumb.label}</Link>
              )}
            </span>
          ))}
        </div>
